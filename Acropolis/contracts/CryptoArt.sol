...
contract CryptoArt is owned {
    ...
    mapping (uint => Offer) public artOfferedForSale;
    ...
    //event Assign(address indexed to, uint256 indexed artIndex);
    event Transfer(address indexed from, address indexed to, uint256 indexed artIndex);
    event ArtTransfer(address indexed from, address indexed to, uint256 indexed artIndex);
    event ArtOffered(uint indexed artIndex, uint indexed minValue, address indexed toAddress);
    event ArtBidEntered(uint indexed artIndex, uint indexed value, address indexed fromAddress);
    ...
    event ArtCreation(uint256 indexed artIndex, address indexed to, string indexed hashOfArt);
    ...
    function CryptoArt() payable {
      ...
      uint256 totalSupply = 0; // Change to the creation of numerous sub coins when a new release of art is conducted
      name = "CRYPTOART";   // Sets the name for display purposes
      symbol = "CART";
      uint decimals=1;  // No decimals for artwork

    }

    function cryptoArtRelease(address to,string hashOfArt) { // Able to accept vector? //Basic minting // Called once for every new artwork
      //if (msg.sender != owner) throw; // May need to remove this.

      artIndexToAddress[nextArtIndexToAssign] = to;
      balanceOf[to]++;

      //Assign(msg.sender, nextArtIndexToAssign);
      ArtCreation(nextArtIndexToAssign, to, hashOfArt);
    ...
    function transferArt(address to, uint artIndex) {
      ...
      if (artOfferedForSale[artIndex].isForSale) {
    ...
    function offerArtForSale(uint artIndex, uint minSalePriceInWei) {
    ...
    function buyArt(uint artIndex) payable {
      Offer offer = artOfferedForSale[artIndex];
      if (artIndex >= nextArtIndexToAssign) throw;
